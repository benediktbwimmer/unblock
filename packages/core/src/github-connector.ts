...
import { nowIso, type ConnectorConnection, type ConnectorExternalMapping } from "./types.js";
...
export const githubIssueMappingInputSchema = z.object({
  ...
  syncDirection: z.enum(["github_to_unblock", "unblock_to_github", "bidirectional"]).default("bidirectional"),
  conflictPolicy: z.enum(["github_wins", "unblock_wins", "last_writer_wins", "operator_review"]).default("operator_review"),
  status: z.enum(["active", "conflict", "operator_review", "archived"]).default("active"),
  metadata: z.record(z.string(), z.unknown()).default({})
});
export type GitHubIssueMappingInput = z.infer<typeof githubIssueMappingInputSchema>;
...
export async function upsertGitHubIssueMapping(store: AppStore, input: GitHubIssueMappingInput): Promise<GitHubIssueMapping> {
  const repo = requireMappingRepository(store);
  const parsed = githubIssueMappingInputSchema.parse(input);
  const existing = await repo.getMappingByExternal(parsed.projectId, parsed.connectionId, "issue", githubIssueExternalId(parsed));
  const now = nowIso();
  const mapping: GitHubIssueMapping = {
    ...
    updatedAt: now,
    archivedAt: parsed.status === "archived" ? (existing?.archivedAt ?? now) : null,
    metadata: {
      ...parsed.metadata,
      repositoryOwner: parsed.repositoryOwner,
      repositoryName: parsed.repositoryName,
...
export async function getGitHubIssueMappingByExternal(
  ...
  projectId: string,
  connectionId: string,
  input: { repositoryOwner: string; repositoryName: string; issueNumber: number }
): Promise<GitHubIssueMapping | null> {
  const repo = requireMappingRepository(store);
  const mapping = await repo.getMappingByExternal(projectId, connectionId, "issue", githubIssueExternalId(input));
...
export async function getGitHubIssueMappingByTask(
  ...
  const repo = requireMappingRepository(store);
  const mapping = await repo.getMappingByLocal(projectId, connectionId, "task", taskId);
  return mapping ? parseGitHubIssueMapping(mapping) : null;
}

export async function listGitHubIssueMappings(
  store: AppStore,
  options: { projectId?: string | undefined; connectionId?: string | undefined; limit?: number | undefined }
): Promise<GitHubIssueMapping[]> {
  const repo = requireMappingRepository(store);
  const mappings = await repo.listMappings({ ...options, provider: "github" });
  return mappings.filter((mapping) => mapping.externalKind === "issue" && mapping.localKind === "task").map(parseGitHubIssueMapping);
}

export function githubIssueExternalId(input: { repositoryOwner: string; repositoryName: string; issueNumber: number }): string {
...
function parseGitHubIssueMapping(mapping: ConnectorExternalMapping): GitHubIssueMapping {
  if (mapping.provider !== "github" || mapping.externalKind !== "issue" || mapping.localKind !== "task") {
...
function requireMappingRepository(store: AppStore): Required<Pick<NonNullable<AppStore["connectors"]>, "upsertMapping" | "getMappingByExternal" | "getMappingByLocal" | "listMappings">> {
  const connectors = store.connectors;
  if (!connectors?.upsertMapping || !connectors.getMappingByExternal || !connectors.getMappingByLocal || !connectors.listMappings) {
    validation("GitHub issue mappings require a store with connector mapping support.");
  }
  return connectors as Required<Pick<NonNullable<AppStore["connectors"]>, "upsertMapping" | "getMappingByExternal" | "getMappingByLocal" | "listMappings">>;
}
